...
// type PartialPick2<T, K extends PropertyKey> = {
...
export const isNotNullable = <T>(value: T): value is ExcludeNullable<T> => {
...
export type IsUnknown<T> = unknown extends T ? true : never;
...
export function arrayOrToArray<T>(arrayOr: undefined | null | T | readonly T[]): readonly T[] {
  if (arrayOr === undefined || arrayOr === null) return [];
  if (Array.isArray(arrayOr)) return arrayOr as any;
  return [arrayOr as any];
}
